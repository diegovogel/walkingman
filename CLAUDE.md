@.claude/diegos-engineering-guidelines.md
...
- **Ports**: slot N → `18900 + 10*N` (web) and `+1` (Vite). Envs start at slot 1
  (18910/18911), then 18920/18921, and so on; slot 0 is the reserved main-ports
  set, unused here. Each env takes the lowest free slot; `destroy` frees it.
- **Inside an env, NEVER run `composer dev` or `npm run dev`**: both are pinned to
  the main checkout's ports and collide. Enforced by
  `scripts/guard-not-in-env.cjs`. Use `./scripts/agent-env.sh serve <name>`
  (background, PID-managed, health-checked; logs in the env's `logs/`). Most tasks
  need only `php artisan test` and `npm run build`; only `serve` when you need a
  live server.
- **Herd serves only the main checkout** at `walkingman.test`. Envs are served by
  `php artisan serve` on their own ports, so don't expect a `.test` domain for an
  env, and don't point Herd at one.
- **Re-anchor after any restart or interruption.** A Claude restart drops the
  ...
  checkout on the default branch, so an env `php artisan test` / `git diff` can run
  ...
  `... run <name> -- php artisan test`), which `cd`s into the env first, so the
  result is cwd-independent. Use it for any cwd-relative command after an
  interruption. Fallback for what `run` doesn't cover: verify `pwd` and
  `git branch --show-current`, then prefer `git -C <worktree>` or an explicit `cd`.
- **Verification scope in an env**: the full suite runs in-env with no extra setup.
  `php artisan test` (Pest: Unit, Feature, Architecture) uses an in-memory SQLite
  database per `phpunit.xml`, so it never touches the env's MySQL schema, and
  `npm run build` needs no server. There is no E2E suite and no fixed-address
  external integration, so there is nothing that can only be exercised on the main
  checkout, and `--main-ports` takeover is deliberately unavailable.
- **Pre-PR workflow**: load the `agent-environments` skill and follow its pre-PR
  ...
  none of it here, so the two can't drift.
- **Cleanup**: after merge, `npm run destroy <name>`. It refuses to remove anything
  dirty or unpushed (commits survive in the main repo's `.git` regardless). Merged
  branches are deleted automatically so the name can be recreated; unmerged
  branches are kept (`create <name> --resume` reattaches one).
- **Dependencies auto-reconcile after a pull.** A committed git hook (`.githooks/`,
  installed by the env system) runs `composer install && npm install` whenever a
  merge/pull/rebase changes `composer.lock` or `package-lock.json`, so the checkout
